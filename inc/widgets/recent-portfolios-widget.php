...
class Recent_Portfolios_Widget extends WP_Widget {
    ...
    function Recent_Portfolios_Widget()
    {
        ...
        $this->WP_Widget('recent_portfolios-widget', __('Venedor: Recent Portfolios', 'venedor'), $widget_ops, $control_ops);
    }
    ...
    {
        $defaults = array('title' => __('Recent Portfolios', 'venedor'), 'number' => 6, 'items' => 1, 'show_title' => 'on', 'show_cats' => 'on');
    ...
    function shortcode($atts, $content) {
        ...
        <?php
            ...
            ob_start();
            if ($count) : ?>
            <div class="entry-related product-slider <?php echo $class ?><?php if (!$title) echo ' notitle' ?><?php if ($desc) echo ' with-desc'; ?> <?php if ($animation_type) echo 'animated' ?>"
