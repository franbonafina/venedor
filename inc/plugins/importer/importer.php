    ...
    if ( current_user_can( 'manage_options' ) && isset( $_GET['import_sample_content'] ) ) {
        ...
        if ( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) { // check for main import class and wp import class
            ...
            if (!isset($_GET['import_step'])) {
                // Import Posts, Pages, Portfolio, FAQ Content, Images, Menus, etc
                $theme_xml = sys_theme_plugins.'/importer/data/dummy_data.xml.gz';
                $importer->fetch_attachments = true;
                ob_start();
                $importer->import($theme_xml);
                ob_end_clean();

                // redirect to next import page
                wp_redirect( admin_url( 'admin.php?page=venedor_import&import_sample_content=true&import_step=2' ) );

            } else if ($_GET['import_step'] == 2) {
                // Import Contact Form 7
                if ( defined( 'WPCF7_VERSION' ) ) {
                    $theme_xml = sys_theme_plugins.'/importer/data/dummy_contactform.xml.gz';
                    $importer->fetch_attachments = true;
                    ob_start();
                    $importer->import($theme_xml);
                    ob_end_clean();
                ...
                if ( class_exists('WooCommerce') ) {
                    $importer = new WP_Import();
                    $theme_xml = sys_theme_plugins.'/importer/data/dummy_products.xml.gz';
                    $importer->fetch_attachments = true;
                    ob_start();
                    $importer->import($theme_xml);
                    ob_end_clean();
                ...
                // Add widgets
                $widgets_json = sys_theme_plugins_uri.'/importer/data/widget_data.json';
