...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( have_comments() ) :
		...
		echo '<p class="add_review"><a href="#review-form" class="inline show-review-form button" title="' . __( 'Add Your Review', 'woocommerce' ) . '">' . __( 'Add Review', 'woocommerce' ) . '</a></p>';
		...
		echo '<p class="noreviews">'.__( 'There are no reviews yet, would you like to <a href="#review-form" class="inline show-review-form">submit yours</a>?', 'woocommerce' ).'</p>';
	...
	if ( get_option('woocommerce_enable_review_rating') == 'yes' ) {
		...
		$comment_form['comment_field'] = '<p class="comment-form-rating input-field"><label for="rating"><span class="fa fa-star"></span>' . __( 'Rating', 'woocommerce' ) . ' <span class="required">*</span></label><select name="rating" id="rating">
