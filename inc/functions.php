...
// add image sizes
function venedor_thumbnails_sizes() {
    global $venedor_settings;
...
function venedor_excerpt($limit = 45, $more_link = true) {
    ...
    if (count($content) >= $limit) {
        array_pop($content);
        if ($post_layout == 'small-alt' || $venedor_settings['blog-excert-type'] == 'html')
    ...
    if ($venedor_settings['blog-excert-type'] == 'html') {
        $content = apply_filters('the_content', $content);
        $content = do_shortcode($content);
    }

    if ($more_link && $post_layout == 'small-alt')
        $content = '<div class="entry-excert">'.$content.'<br/><a class="btn more-links" href="'.esc_url( apply_filters( 'the_permalink', get_permalink() ) ).'">'.__('Read More', 'venedor').'</a></div>';
    else if ($more_link)
        $content = '<div class="entry-excert">'.$content.' <a class="inline more-links" href="'.esc_url( apply_filters( 'the_permalink', get_permalink() ) ).'">'.__('Read More', 'venedor').'...</a></div>';
...
function venedor_add_url_parameters($url, $name, $value) {
    ...
    return venedor_build_url($url_data);
}
endif;
...
function venedor_breadcrumbs() {
    global $venedor_settings, $post, $wp_query;
    
    $delimiter = '<span>/</span>';
    if ($venedor_settings['breadcrumbs-separator']) {
        $delimiter = '<span>'. ($venedor_settings['breadcrumbs-separator'] != '>' ? $venedor_settings['breadcrumbs-separator'] : '&gt;') .'</span>';
    }
    ...
    $shop_page_id = false;
    $shop_page = false;
    $fron_page_shop = false;
    if ( defined( 'WOOCOMMERCE_VERSION' ) ) {
        $permalinks   = get_option( 'woocommerce_permalinks' );
        $shop_page_id = wc_get_page_id( 'shop' );
        $shop_page    = get_post( $shop_page_id );
        $fron_page_shop = get_option( 'page_on_front' ) == wc_get_page_id( 'shop' );
    }
    ...
    if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && $fron_page_shop ) ) || is_paged() ) {
        echo '<ul>';
            ...
            if ( get_post_type() == 'product' ) {
                ...
                if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
                    ...
                    foreach ( $ancestors as $ancestor ) {
                        ...
                        echo $before . '<a href="' . get_term_link( $ancestor->slug, 'product_cat' ) . '">' . $ancestor->name . '</a>' . $delimiter . $after;
                    }

                    echo $before . '<a href="' . get_term_link( $main_term->slug, 'product_cat' ) . '">' . $main_term->name . '</a>' . $delimiter . $after;
    ...
    <div class="toolbar clearfix">
        <div class="sorter clearfix">
            <div class="sort-by">
                ...
                <div class="dropdown left dropdown-select">
                    ...
                    <ul class="dropdown-menu">
                        <?php if ($orderby != 'default') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'default') ?>"><?php echo __('Default', 'venedor') ?></a></li>
                        <?php endif; ?>
                        <?php if ($orderby != 'popularity') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'popularity') ?>"><?php echo __('Popularity', 'venedor') ?></a></li>
                        <?php endif; ?>
                        <?php if ($orderby != 'rating' && get_option( 'woocommerce_enable_review_rating' ) != 'no') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'rating') ?>"><?php echo __('Rating', 'venedor') ?></a></li>
                        <?php endif; ?>
                        <?php if ($orderby != 'date') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'date') ?>"><?php echo __('Date', 'venedor') ?></a></li>
                        <?php endif; ?>
                        <?php if ($orderby != 'price') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'price') ?>"><?php echo __('Price', 'venedor') ?></a></li>
                        <?php endif; ?>
                        <?php if ($orderby != 'name') : ?>
                            <li><a tabindex="-1" href="<?php echo venedor_add_url_parameters($query_string, 'product_orderby', 'name') ?>"><?php echo __('Name', 'venedor') ?></a></li>
                        <?php endif; ?>
                    </ul>
                </div>
                ...
                <?php // order ?>
                <?php if($order == 'desc'): ?>
                    <a class="btn-arrow order-desc" href="<?php echo venedor_add_url_parameters($query_string, 'product_order', 'asc') ?>"></a>
                <?php else: ?>
                    <a class="btn-arrow order-asc left" href="<?php echo venedor_add_url_parameters($query_string, 'product_order', 'desc') ?>"></a>
                <?php endif; ?>
            </div>
            
            <div class="view-mode gridlist-toggle">
                <a href="#" id="grid" title="<?php echo __('Grid View', 'venedor') ?>"></a><a href="#" id="list" title="<?php echo __('List View', 'venedor') ?>"></a>
            </div>
        </div>
...
function venedor_woocommerce_thumbnail() {
    ...
    if (!$thumb_image) {
        if ( woocommerce_placeholder_img_src() )
            $thumb_image = woocommerce_placeholder_img( $size );
    }
    ...
    if ($venedor_settings['product-price']) {
        ...
        echo '<div class="price-box '. $venedor_settings['product-price-pos'] . ($handler == 'variable' ? ' hide-del' : '') . '">';
...
function venedor_single_product_rating() {
    ...
    <div class="ratings" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"><meta content="<?php echo $rating; ?>" itemprop="ratingValue" /><meta content="<?php echo $product->get_rating_count(); ?>" itemprop="ratingCount" />
        ...
        <span class="amount">
            ...
            if (!isset($venedor_quickview)) : ?>
                <a href="#reviews" id="goto-reviews"><?php echo $product->get_rating_count() . ' ' . __('Reviews', 'venedor'); ?></a><span class="gap">|</span><a href="#review-form" id="goto-review-form"><?php echo __('Add Your Review', 'venedor') ?></a>
            <?php else : ?>
                <?php echo $product->get_rating_count() . ' ' . __('Reviews', 'venedor'); ?>
            <?php endif; ?>
        </span>
    </div>
    <?php endif;
            ...
            <div class="cart_list_wrap scrollbar-rail">
                ...
                <?php foreach (WC()->cart->cart_contents as $cart_item_key => $cart_item) { ?>
                    ...
                    <li class="item clearfix">
                        <?php echo '<a class="product-image" href="'.get_permalink($cart_item['product_id']).'">' . $_product->get_image().'</a>'; ?>
                        <div class="product-details">
                            <?php
                            $product_title = $_product->get_title();
                            echo '<a class="product-name" href="'.get_permalink($cart_item['product_id']).'">' . apply_filters('woocommerce_cart_widget_product_title', $product_title, $_product) . '</a>';
                            echo '<span class="qty">'.$cart_item['quantity'].' &times; </span>';
...
function venedor_default_checkout_fields($fields) {
    ...
    if (isset($fields['account']) && isset($fields['account']['account_password-2']) && $fields['account']['account_password-2']) {
        $fields['account']['account_password-2'] = array(
            'type'                 => 'password',
            'label'             => '<span class="fa fa-lock"></span>'.__( 'Confirm password', 'woocommerce' ),
...
function venedor_single_product_links() {
    ...
    if ( $wishlist || $compare || (defined('ADDTHIS_INIT') && $venedor_settings['product-addthis'])) : ?>
        <div class="add-links<?php if ( $wishlist && $compare ) echo ' show-all' ?>">
            ...
            ?>
            <?php if (defined('ADDTHIS_INIT') && $venedor_settings['product-addthis']) : ?>
                <div class="addthis-icons clearfix">
                    <?php if (get_the_content('')) : ?><span class="share"><?php echo __('Share', 'venedor') ?>: </span><?php endif; ?>
                    <?php
                    // addthis buttons
    ...
    if ( $products->have_posts() ) : ?>
        ...
        <div class="product-featured-slider owl-carousel">
            ...
            <?php while ( $products->have_posts() ) : $products->the_post(); 
                ...
                <div class="container product-item"><div class="row">
                ...
                <div class="container"><div class="row">
                    ...
                    <div class="col-sm-<?php echo ($thumb_image)?'7':'12' ?>">
                        <div class="category-details<?php echo (!$thumb_image)?' noimage':'' ?>">
                            ...
                            <?php if ($more_link) : ?>
                                <a class="fancybox btn btn-inverse btn-lg more-links" href="#term-description"><?php _e('Learn More', 'venedor') ?></a>
...
function venedor_banner($banner_class = '') {
    ...
    <?php } else if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && $banner_type === 'product_slider' && isset($product_slider)) { ?>
...
function venedor_html_minicart() {
    ...
    if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) :
        ...
        <div id="mini-cart" class="dropdown<?php if ($venedor_settings['minicart-pos'] == 'middle') echo ' middle' ?>">
            <div class="dropdown-toggle cart-head" data-toggle="dropdown" data-hover="dropdown" data-delay="1000" data-close-others="false">
                <span class="cart-icon">
                    <span class="glyphicon glyphicon-shopping-cart"></span>
                </span><span class="cart-details">
                    <span class="cart-items"><?php echo ($_cartQty > 0) ? $_cartQty : '0' ?> <?php _e('item(s)', 'venedor') ?></span>
                    <span class="mobile-hide"> - <?php echo $woocommerce->cart->get_cart_total(); ?></span>
                </span>
            </div>
                ...
                <div class="block-content cart-content">
                    ...
                    if ($_cartQty == 0) :
                                    ...
                                    if ($_product->exists() && $cart_item['quantity']>0) : ?>
                                        <li class="item">
                                            ...
                                            <div class="product-details">
                                                ...
                                                $product_title = $_product->get_title();
                                                echo '<a class="product-name" href="'.get_permalink($cart_item['product_id']).'">' . apply_filters('woocommerce_cart_widget_product_title', $product_title, $_product) . '</a>';
                                                echo '<span class="quantity">'.$cart_item['quantity'].' &times; </span>';
                                                echo '<span class="price">'.woocommerce_price($_product->get_price()).'</span>';
                                                ?>
                                            </div>
                                            <?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove remove-product" title="%s" data-cart_id="%s">&times;</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __('Remove this item', 'woocommerce'), $cart_item_key ), $cart_item_key ); ?>
                        ...
                        <div class="minicart-actions clearfix">
                            <div class="total">
                                <span class="total-label"><?php echo __('Subtotal', 'woocommerce') ?>: </span>
                                <span class="subtotal"><?php echo $woocommerce->cart->get_cart_total(); ?></span>
                            </div>
                            <div class="buttons">
