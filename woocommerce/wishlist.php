...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    //{ echo apply_filters( 'yith_wcwl_wishlist_title', '<h2 class="entry-title">' . $wishlist_title . '</h2>' ); }
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        <thead>
            <tr class="mobile-hide">
                ...
                <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : $cols++; ?><th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'yit' ) ?></span></th><?php endif ?>
                <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : $cols++; ?><th><span class="nobr"><?php _e( 'Stock Status', 'yit' ) ?></span></th><?php endif ?>
                <th><span class="nobr"></th>
            </tr>
            <?php if ( count( $wishlist ) > 0 ) : ?>
            <tr class="mobile-row">
                <th colspan="<?php echo $cols ?>"><?php echo count( $wishlist ); echo ' '; _e( 'Product(s) In Your Wishlist', 'venedor' ); ?></th>
            </tr>
            <?php endif; ?>
        </thead>
        <tbody>
                    ...
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        ...
                        <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                            <td class="product-price">
                                <span class="mobile-show"><?php _e('Unit Price', 'yit') ?>: </span>
                                <?php
                        ...
                        <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                            <td class="product-stock-status">
                                <span class="mobile-show"><?php _e('Stock Status', 'yit') ?>: </span>
                                <?php
                        ...
                        <?php if( get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) : ?>
                            <td class="product-add-to-cart">
                                ...
                                <div class="product-remove"><div><a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="btn-arrow remove" title="<?php _e( 'Remove this product', 'yit' ) ?>">&times;</a></div></div>
                            </td>
                        <?php  else : ?>
                            <?php $remove_wishlist = esc_attr( "remove_item_from_wishlist( '" . esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) ) . "', 'yith-wcwl-row-" . $values['ID'] ."');" ); ?>
                            <td class="product-remove"><div><a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="btn-arrow remove" title="<?php _e( 'Remove this product', 'yit' ) ?>">&times;</a></div></td>
                        <?php endif ?>
                    </tr>
                <?php
                endif;
