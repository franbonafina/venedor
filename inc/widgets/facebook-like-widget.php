...
class Venedor_Facebook_Like_Widget extends WP_Widget {
	...
	function Venedor_Facebook_Like_Widget()	{
		...
		$this->WP_Widget('facebook-like-widget', __('Venedor: Facebook Like Box', 'venedor'), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance) {
        ...
        if ($fb_name && $limit > 0) {
            ...
            if ($title) {
                $title .= '<a class="fb-like btn btn-special" href="http://www.facebook.com/'. $fb_name . '" target="_blank"><span class="fa fa-hand-o-right"></span> '. __('Like', 'venedor') .'!</a>';
            ...
            if ($fb_name && $limit > 0) {
                ...
                try {
                    if ( false === ( $html = get_transient($trans_key) ) ) {
                        ...
                        $fb_profile = json_decode(file_get_contents('http://graph.facebook.com/' . $fb_name), true);
                        if(empty($fb_profile['id'])){// invalid facebook name
                            $html = __('Invalid Facebook Name', 'venedor');
                        } else {
                            $result = array();

                            $ret = array();
                            $matches = array();
                            $url = 'https://www.facebook.com/plugins/likebox.php?href=https://www.facebook.com/'.$fb_profile['id'].'&connections='.$limit;

                            $html = '<p>'. $fb_profile['likes'] .' '. __('people like', 'venedor') .' <a target="_blank" href="'. $fb_profile['link'] .'" title="'. $fb_profile['name'] .'">'. $fb_profile['name'] .'</a></p>';
                            ...
                            if ($doc) {
                                $html .= '<div class="clearfix fb-persons">';
